...
LPCWSTR charToLPCWSTR(const char* charArray) {
	size_t size = strlen(charArray) + 1;
	wchar_t* wcharArray = new wchar_t[size];
	size_t convertedChars = 0;
	mbstowcs_s(&convertedChars, wcharArray, size, charArray, _TRUNCATE);
	return wcharArray;
}
...
{
	LPCWSTR titleLPCWSTR = charToLPCWSTR(Title);
	SetConsoleTitle(titleLPCWSTR);
}
